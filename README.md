...
- `Order` (주문) 개념을 도입하여 `Cart`와 분리
    - 장바구니에 담겨있는 상태와 할인까지 가격이 반영되어 계산이 이루어지는 상태는 엄연히 다르다고 생각
    - 주문 및 할인 정책을 반영한 가격 계산 기능을 `Order`로 이전하기
    - 가격이 반영되어 계산이 실제로 이루어진 데이터 관리를 대비하기 위한 `Order` 클래스 설계
- `view.KioskConsole` 클래스의 출력 관련 메소드들 정리
    - 현재 `printxxx()`, `displayxxx()`등 메소드명으로 구별이 되어 있지만 공통적인 부분이 여전히 존재
    - 추상화 및 모듈화 그리고 사용 시 모듈화된 메소드를 조립하여 사용하도록 하여 재사용성 제고
- `View` 인터페이스 도입을 통해 `Kiosk`와의 결합도를 낮추기
    - 콘솔 관련 메소드가 `view.KioskConsole`로 모두 옮겨졌지만, 여전히 결합도가 높음
    - 클라이언트의 개발 방법이 변경될 경우 Kiosk의 관련 코드를 모두 수정해야 함
    - 따라서 인터페이스를 도입하고 필수 구현 메소드를 정의하여 변경에 열려있도록 개선
- `ScreentIntent`이 본래 `Android`의 인텐트처럼 다양한 타입의 값 전달
    - 현재는 사용자가 선택한 기능의 `int`형 `index`만 전달함
    - 차후 확장을 고려하여 `Generic`혹은 `Object`타입을 이용하여 여러 타입의 데이터를 전달할 수 있도록 개선